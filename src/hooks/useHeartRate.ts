 ...
 *
 * ── NETTOYAGE (ScanEnhancer retiré) ────────────────────────────────────────
 * Toute référence à ScanEnhancer / scanEnhancements a été supprimée :
 *   - import "@/lib/scanEnhancements" retiré
 *   - useRef(new ScanEnhancer()) retiré (il était au niveau module → écran blanc)
 *   - appels scanRef.current.onFrameData() / .onBeat() retirés
 * Le moteur natif ci-dessous (BPM spectral + VFC gatée) est la SEULE source de
 * vérité. Fichiers désormais orphelins, supprimables :
 *   src/lib/scanEnhancements.ts, src/lib/torchController.ts,
 *   src/lib/ppgQuality.ts, src/lib/hrvMetrics.ts
 * (Garder src/lib/feedback.ts : utilisé ailleurs — useFeedback, tap global.)
 *
